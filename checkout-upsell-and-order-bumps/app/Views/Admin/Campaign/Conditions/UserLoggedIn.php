...
<div class="condition-method flex-fill">
    <select class="form-control optional trigger-change" name="conditions[<?php echo esc_attr($key); ?>][value]">
        <option value="yes" <?php if ($value == 'yes') echo "selected"; ?>><?php esc_html_e("Yes", 'checkout-upsell-and-order-bumps'); ?></option>
        <option value="no" <?php if ($value == 'no') echo "selected"; ?>><?php esc_html_e("No", 'checkout-upsell-and-order-bumps'); ?></option>
    </select>
</div>